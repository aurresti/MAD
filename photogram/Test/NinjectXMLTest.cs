...
namespace Es.Udc.DotNet.Photogram.Test
{
    ...
    public class NinjectTest
    {
        ...
        public void NinjectTestMethod()
        {
            ...
            IUserDao dao = kernel.Get<IUserDao>();

            Assert.IsInstanceOfType(dao, typeof(IUserDao));

            IImageDao daoI = kernel.Get<IImageDao>();

            Assert.IsInstanceOfType(daoI, typeof(IImageDao));

            ICategoryDao daoC = kernel.Get<ICategoryDao>();

            Assert.IsInstanceOfType(daoC, typeof(ICategoryDao));

            ICommentDao daoCo = kernel.Get<ICommentDao>();

            Assert.IsInstanceOfType(daoCo, typeof(ICommentDao));
